...
namespace :file_adder do
  ...
  task :compress do
    all_video_commands {|file_name|
      ...
      comand = "ffmpeg -n -i #{Rails.root.join('data', file_name)} -an -crf 10 -vf scale=-1:480 #{Rails.root.join('data', file_name_without_extension(file_name))}.webm"
      system(comand)
    }
  end
