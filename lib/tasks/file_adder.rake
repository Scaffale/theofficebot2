...
namespace :file_adder do
  ...
  task :compress do
    Parallel.each(all_videos, in_processes: 4) do |file_name|
      ...
      speed = 'veryslow'
      comand = "ffmpeg -n -loglevel info -i #{Rails.root.join('data',
                                                              file_name)} -codec:v libx264 -preset #{speed} -an -crf 32 -vf \"subtitles=#{Rails.root.join('data',
                                                                                                                                                          file_name_clean)}.srt:force_style='Fontsize=28', scale=trunc(oh*a/2)*2:480, fps=24\" #{Rails.root.join('data',
                                                                                                                                                                                                                                                                 file_name_clean)}_#{speed}.mp4"
      system(comand)
    end
  end
